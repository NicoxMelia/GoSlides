...
export const ICON_LIBRARIES: Array<{id:IconLibrary;name:string;description:string}> = [
  ...
  {id:'tech',name:'Tecnología',description:'Lenguajes, cloud, DevOps y herramientas'},
];

const keys: Record<Exclude<IconLibrary, 'tech'>, Record<IconName,string>> = {
...
const modules: Record<Exclude<IconLibrary, 'tech'>, Record<string,unknown>> = {
...
export function IconGlyph({ name, library='lucide', size=32, strokeWidth=1.8, brandColors=true }: { name:string; library?:IconLibrary; size?:number|string; strokeWidth?:number; brandColors?:boolean }) {
  if (library === 'tech') {
