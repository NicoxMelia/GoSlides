...
export function Block({ block, assets, step = 999 }: { block: SlideBlock; assets: Record<string, string>; step?: number }) {
  ...
  if (block.type === 'code') return <CodeBlockView code={block.code} language={block.language} title={block.title} frameStyle={block.frameStyle} codeTheme={block.codeTheme} showLineNumbers={block.showLineNumbers} showWindowControls={block.showWindowControls} simulationEnabled={block.simulationEnabled} simulationOutput={block.simulationOutput}/>;
...
function CanvasItem({ element, assets, embedded=false }: { element: CanvasElement; assets: Record<string, string>; embedded?: boolean }) {
  ...
  if (element.type === 'code') return <div className="canvas-render-item canvas-code" style={canvasStyle(element,embedded)}><CodeBlockView code={element.code} language={element.language} title={element.title} frameStyle={element.frameStyle} codeTheme={element.codeTheme} showLineNumbers={element.showLineNumbers} showWindowControls={element.showWindowControls} simulationEnabled={element.simulationEnabled} simulationOutput={element.simulationOutput}/></div>;
  ...
  if (element.type === 'icon') return <div className={`canvas-render-item canvas-icon ${element.style?.sketch?'element-sketch':''}`} style={canvasStyle(element,embedded)}><IconGlyph name={element.name} library={element.library} brandColors={element.brandColors} size="55%" />{element.label && <small>{element.label}</small>}</div>;
